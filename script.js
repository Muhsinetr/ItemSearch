...
function csvToArray(csv) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        let SPriceCovert = covertPPrice(eachData[8])
        let WPriceCovert = covertPPrice(eachData[9])
        let lineDatas = {}
        lineDatas.itemName = eachData[1];
        lineDatas.PPrice = PPriceCOnvert;
        lineDatas.SPrice = SPriceCovert;
        lineDatas.WPrice = WPriceCovert;
...
const spDisplay = document.getElementById('special');
const whDisplay = document.getElementById('whole');
...
function showItemDetails(item) {
    ...
    spDisplay.textContent = `Sp : ${item.SPrice}`
    whDisplay.textContent = `whole : ${item.WPrice}`
